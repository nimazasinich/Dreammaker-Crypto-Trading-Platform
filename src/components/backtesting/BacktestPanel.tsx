    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/backtest/run`, {
        ...
        body: JSON.stringify({
          symbol,
          strategy: 'default',
          startDate: new Date(typeof first === 'number' ? first : first.getTime()).toISOString(),
          endDate: new Date(typeof last === 'number' ? last : last.getTime()).toISOString(),
      ...
      const res = apiResult.data?.result || apiResult.data;
