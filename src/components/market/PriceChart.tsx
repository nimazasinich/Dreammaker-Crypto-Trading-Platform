...
export const PriceChart: React.FC<PriceChartProps> = ({ 
  ...
  const fetchChartData = async () => {
    ...
    try {
      ...
      // Use the symbol AS IS - don't modify it
      // The symbol comes correctly from parent (e.g., "BTC", "ETH", etc.)
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        // Validate candles have reasonable price (relaxed for HF data)
        const symbolName = symbol.toUpperCase();
        const minReasonablePrice = symbolName.includes('BTC') ? 1000 :  // Relaxed from 10000
                                   symbolName.includes('ETH') ? 100 :    // Relaxed from 1000
                                   symbolName.includes('BNB') ? 10 :     // Relaxed from 100
                                   0.01;

        const validCandles = candles.filter(c => {
          // Check if candle has valid structure
          const hasValidData = c.open > 0 && c.high > 0 && c.low > 0 && c.close > 0 &&
                              c.high >= c.open && c.high >= c.close &&
                              c.low <= c.open && c.low <= c.close;
          
          // Check if price is reasonable (not mock data)
          const hasReasonablePrice = c.close >= minReasonablePrice;
          
          return hasValidData && hasReasonablePrice;
        });

        if (validCandles.length > 10) { // Need at least 10 candles for chart
          if (import.meta.env.DEV) logger.info(`✅ Loaded ${validCandles.length} valid candles for ${symbol}`);
          setChartData(validCandles);

          const calculatedIndicators = calculateIndicators(validCandles);
          setIndicators(calculatedIndicators);
        } else {
          // Data quality issue - hide chart
          logger.warn(`⚠️ HF Space returned ${validCandles.length} valid candles (need 10+). Data appears to be mock/test data.`);
          setChartData([]);
          setIndicators(undefined);
        }
      } else {
        // NO DATA - Show empty chart
        logger.warn(`⚠️ No data available for ${symbol}`);
        setChartData([]);
        setIndicators(undefined);
      }
    ...
    <div className="bg-gray-900 rounded-lg border border-gray-800">
      ...
      <div className="p-4">
        <div className="relative h-80 bg-gray-950 rounded border border-gray-800 overflow-hidden">
          <svg width="100%" height="100%" viewBox="0 0 100 300" preserveAspectRatio="none" className="absolute inset-0">
            {/* Grid lines */}
            <defs>
            ...
            {chartType === 'candlestick' ? (
              (validData || []).map((candle, index) => {
                ...
                const bodyHeight = Math.abs(yOpen - yClose) || 0.3;
                const bodyTop = Math.min(yOpen, yClose);
                
                // Calculate dynamic candle width - much thinner for professional look
                const candleWidth = Math.max(0.15, Math.min(0.5, 60 / validData.length));

                return (
                  <g key={index}>
                    {/* Wick (shadow) - very thin */}
                    <line
                      x1={`${x}%`}
                      y1={yHigh}
                      x2={`${x}%`}
