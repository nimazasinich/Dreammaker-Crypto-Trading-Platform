  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const result = await performanceMonitor.measure(
        'fetchOHLC',
        async () => {
          // ✅ استفاده از endpoint تست شده Hugging Face: /api/ohlcv
          const url = `${API_BASE}/api/ohlcv?symbol=${binanceSymbol}&timeframe=${timeframe}&limit=${limit}`;

          logger.info('Fetching OHLC from Hugging Face:', { symbol: binanceSymbol, timeframe, limit });
