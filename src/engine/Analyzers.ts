...
export function analyzeHarmonic(ohlc: OHLC[]): DetectorScore {
  ...
  try {
    ...
    let score = 0.5 + ((bestPattern.reliabilityScore || 0.5) - 0.5) * 0.3;
    score += ((bestPattern.completionProbability || 0.5) - 0.5) * 0.2;

    // PRZ confluence adds weight
    score += ((bestPattern.prz.confluence || 0.5) - 0.5) * 0.2;
    ...
    return {
      ...
      meta: {
        ...
        reliability: bestPattern.reliabilityScore || 0.5,
        completion: bestPattern.completionProbability || 0.5,
        przConfluence: bestPattern.prz.confluence || 0.5,
