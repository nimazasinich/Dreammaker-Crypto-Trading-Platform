...
// SECURITY: All API keys now loaded from environment variables only
// No hardcoded secrets in code - use env.example as template
const EMBEDDED_KEYS = {
  CMC: process.env.CMC_API_KEY || process.env.COINMARKETCAP_API_KEY || '',
  ETHERSCAN: process.env.ETHERSCAN_API_KEY || process.env.Etherscan || '',
  ETHERSCAN_BACKUP: process.env.ETHERSCAN_BACKUP_API_KEY || process.env.Etherscan_2 || '',
  BSCSCAN: process.env.BSCSCAN_API_KEY || process.env.BscScan || '',
  CRYPTOCOMPARE: process.env.CRYPTOCOMPARE_API_KEY || '',

  // Optional API keys - always from environment
  MESSARI: process.env.MESSARI_API_KEY || '',
  SANTIMENT: process.env.SANTIMENT_API_KEY || '',
  COINMETRICS: process.env.COINMETRICS_API_KEY || '',
  HUGGINGFACE: process.env.HUGGINGFACE_TOKEN || process.env.HUGGINGFACE_API_KEY || process.env.HF_TOKEN || '',
};
...
export async function getMarketData(query: string, opts: { providers?: Array<'coinpaprika' | 'coincap' | 'defillama' | 'coingecko'> } = {}): Promise<MarketQuote> {
  const order = opts.providers ?? ['coinpaprika', 'coincap', 'coingecko', 'defillama'];
  const errors: any[] = [];
