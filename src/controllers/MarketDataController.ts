    ...
    try {
      ...
      if (useRealData) {
        ...
        const prices = await this.cache.getOrSet(
          cacheKey,
          async () => {
            return await this.multiProviderService.getRealTimePrices(symbolList);
          },
  ...
  async getCurrentPrice(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (this.config.isRealDataMode) {
        ...
        const priceData = await this.cache.getOrSet(
          cacheKey,
          async () => {
            return await this.multiProviderService.getRealTimePrice(cleanSymbol);
          },
