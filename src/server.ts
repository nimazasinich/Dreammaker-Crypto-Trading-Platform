...
if (process.env.FRONTEND_ORIGIN) {
  ...
  logger.warn('⚠️ SECURITY WARNING: FRONTEND_ORIGIN not set in HuggingFace deployment');
  logger.warn('⚠️ Falling back to permissive CORS - SET FRONTEND_ORIGIN env var!');
  ...
  logger.error('❌ CRITICAL SECURITY ERROR: FRONTEND_ORIGIN not set in production!');
  logger.error('❌ CORS will block all requests. Set FRONTEND_ORIGIN in environment.');
  ...
  logger.warn('⚠️ Consider setting FRONTEND_ORIGIN env var even in development');
}
...
app.use((req, res, next) => {
  ...
  if (origin && Array.isArray(corsOrigins) && !corsOrigins.includes(origin)) {
    logger.warn('⚠️ CORS blocked request from unauthorized origin', {
      origin,
      path: req.path,
      method: req.method,
      ip: req.ip
...
logger.info('✅ Rate limiting and input validation enabled on /api routes');
...
app.post('/api/ai/predict', async (req, res) => {
  try {
    logger.info('Deprecated endpoint /api/ai/predict hit — redirecting to /api/ml/predict');
...
// Backtesting endpoint (deprecated alias) — redirect to unified route module
app.post('/api/ai/backtest', async (req, res) => {
  try {
    logger.info('Deprecated endpoint /api/ai/backtest hit — redirecting to /api/backtest/run');
...
// (Removed duplicate /api/ai/backtest implementation — consolidated via redirect above)
...
logger.info('✅ HuggingFace proxy routes configured');
logger.info('✅ Binance and CoinGecko proxy routes configured');
