...
test('RiskManagementView — test rules & scenario run', async ({ page }) => {
  const finish = attachEnvelopeChecker(page);
  await page.goto('http://localhost:3000/risk-management', { waitUntil:'networkidle' });
  const controls = await page.$$('[role="button"], button, select');
